...
def get_traffic_info():
    ...
    if not api_key:
        # 데모용 샘플 데이터
        return get_sample_traffic_data()
...
def get_sample_traffic_data():
    ...
    roads_data = [
        {"road_name": "한남대로", "traffic_level": traffic_level, "impact": 1.2 if traffic_level == "혼잡" else 1.0},
        {"road_name": "이태원로", "traffic_level": traffic_level, "impact": 1.1 if traffic_level == "혼잡" else 1.0},
        {"road_name": "보광로", "traffic_level": "원활", "impact": 0.9},
        {"road_name": "한강대로", "traffic_level": traffic_level, "impact": 1.3 if traffic_level == "혼잡" else 1.0}
    ]
...
def analyze_traffic_impact(roads_data):
    ...
    recommendation = get_traffic_recommendation(total_impact, congested_roads, smooth_roads)
...
if __name__ == "__main__":
    ...
    for road in traffic['roads']:
        print(f"  {road['road_name']}: {road['traffic_level']} (영향도: {road.get('impact', 1.0)}배)")
